...
if (isset($_POST['news_view']) && !empty($_POST['news_view'])) {
...
<div class="news-popup">
    <div class="wrap6">
        ...
        <div class="img-popup-wrap">
            ...
            <div class="card">
                <span id="responseSubmitnews"></span>
                <div class="card-header">
                    <h5 class="card-text">News</h5>
                    <p class="card-text">Add News ? Please fill details below.</p>
                </div>
                      ...
                      <div class="form-row">
                        ...
                        <div class="col">
                            <div class="form-group">
                              <select class="form-control" name="categories_news" id="categories_news">
                                <option value="">Select what types of News</option>
...
if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
    ...
    if (!empty($_FILES['video']['name'][0])) {
      $video= $_FILES['video'];
      $video_ = $home->uploadnewsFile($video);
    ...
    $categories_news=  $users->test_input($_POST['categories_news']);
  ...
  if (!empty($_POST['photo-Title5'])) {
	...
	if (!empty($title) || !empty(array_filter($photo['name'])) || !empty(array_filter($other_photo['name'])) ) {
		if (!empty($photo['name'][0])) {
			# code...
			$photo_ = $home->uploadnewsFile($photo);
      $other_photo_ = $home->uploadnewsFile($other_photo);
		}
	...
	$users->Postsjobscreates('news',array( 
	'title'=> $title,
	'authors'=> $authors,
	'photo'=> $photo_, 
    ...
    // 'photo_Title1'=> $photo_Title1,
    // 'photo_Title2'=> $photo_Title2,
    // 'photo_Title3'=> $photo_Title3,
    // 'photo_Title4'=> $photo_Title4,
    // 'photo_Title5'=> $photo_Title5,
    'user_id3'=> $user_id,

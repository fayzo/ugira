...
if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'viewORedituncomment') {
		...
		if ($_POST['key'] == 'fetch_array') {
			...
			$sql = $conn->query("SELECT * FROM comment WHERE approved='off' ORDER BY date DESC LIMIT $begin_nmber, $end_nmber ");
			if ($sql->num_rows > 0) {
				$response = "";
				$increment=0;
				   ...
				   $increment++;
					$response .= '
						<tr>
							...
							<td>
								<input type="button" onclick="viewORedituncomment_('.$row["post_id"].', \'edit\')" value="Edit" class="btn btn-primary">
								<input type="button" onclick="viewORedituncomment_('.$row["post_id"].', \'view\')" value="View" class="btn">
								<input type="button" onclick="deleteRowuncomment_('.$row["post_id"].')" value="Delete" class="btn btn-danger">
								<input type="button" onclick="approved('.$row["post_id"].')" value="Approvel" class="btn btn-warning">
							</td>
						</tr>
					';
				}
		...
		if ($_POST['key'] == 'deleteRowuncomment') {
			$conn->query("DELETE FROM comment WHERE post_id='$rowID'");
			exit('The Row Has Been Deleted!');
		}

	    if ($_POST['key'] == 'approved') {
	 		$conn->query("UPDATE comment SET approved='on' WHERE post_id='$rowID'");
			exit('success');
		}
